 ...
 * 
 * Built to work with Redux theme options framework
 * @ThemezKitchen
 * 
 * @since 1.0.0
 *
...
class Featured_Content {
	...
	public static function init() {
                ...
                if (isset($neodent_theme_settings['max-posts']) && !empty($neodent_theme_settings['max-posts'])) : 
                    $mx_posts = $neodent_theme_settings['max-posts'];
                endif;
                
                 if (isset($neodent_theme_settings['tag-name']) && !empty($neodent_theme_settings['tag-name'])) : 
                    $tg_name = $neodent_theme_settings['tag-name'];
                endif;
                
                if (isset($neodent_theme_settings['hide-tag']) && !empty($neodent_theme_settings['hide-tag'])) : 
                    $hde_tag = $neodent_theme_settings['hide-tag'];
                endif;
		...
		//$theme_support = get_theme_support( 'featured-content' );
                $theme_support = $featured_options;
		...
		add_action( 'admin_enqueue_scripts', array(__CLASS__, 'adJsFileAdmin') );
        ...
        public static function adJsFileAdmin() {
            wp_enqueue_script( 'featured-content-suggest', get_template_directory_uri() . '/js/featured-content-admin.js', array( 'jquery', 'suggest' ), '20141022', true );
        }
	...
	public static function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content', array(
			'title'          => __( 'Featured Content', 'neodent' ),
			'description'    => sprintf( __( 'Use a <a href="%1$s">tag</a> to feature your posts. If no posts match the tag, <a href="%2$s">sticky posts</a> will be displayed instead.', 'neodent' ),
				esc_url( add_query_arg( 'tag', _x( 'featured', 'featured content default tag slug', 'neodent' ), admin_url( 'edit.php' ) ) ),
		...
		$wp_customize->add_setting( 'featured-content[tag-name]', array(
			'default'              => _x( 'featured', 'featured content default tag slug', 'neodent' ),
			'type'                 => 'option',
		...
		$wp_customize->add_control( 'featured-content[tag-name]', array(
			'label'    => __( 'Tag Name', 'neodent' ),
			'section'  => 'featured_content',
			'priority' => 20,
		) );
		$wp_customize->add_control( 'featured-content[hide-tag]', array(
			'label'    => __( 'Don&rsquo;t display tag on front end.', 'neodent' ),
			'section'  => 'featured_content',
			'type'     => 'checkbox',
			'priority' => 30,
		) );
	...
	public static function get_setting( $key = 'all' ) {
                ...
                //take saved from redux theme options
                global $neodent_theme_settings;
                ...
                $saved = $featured_options;
                //TEST
                //$saved = (array) get_option( 'featured-content' );
		$defaults = array(
			'hide-tag' => 1,
			'tag-id'   => 0,
			'tag-name' => _x( 'featured', 'featured content default tag slug', 'neodent' ),
		);
