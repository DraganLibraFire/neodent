...
/**
 * Verteez Embed Grabber - A script for grabbing media related to a post.
 *
 * Embed Grabber is a script for pulling media either from the post content or attached to the 
 * post.
 */

/**
 * Wrapper function for the neodent_Media_Grabber class.  Returns the HTML output for the found media.
 *
 * @since  1.0.0
 * @access public
 * @param  array
 ...
 * @category helpers
 */
function neodent_embed_grab( $args = array() ) {

	$media = new neodent_Embed_Grab( $args );
...
class neodent_Embed_Grab {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'wk_media_grabber_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'wk_media_grabber_autoembed_media',       array( $wp_embed, 'autoembed' ) );
		...
		$this->args    = apply_filters( 'wk_media_grabber_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'wk_media_grabber_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'wk_media_grabber_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'wk_media_grabber_autoembed_media', $value[0] ) );
